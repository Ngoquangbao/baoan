...
import { assets } from '../assets/assets'; // Removed trailing slash
...
const Sidebar = () => {
  ...
  return (
    <div className='w-[25%] h-full p-2 flex-col gap-2 text-white hidden lg:flex'>
      <div className='bg-[#121212] h-[15%] rounded flex flex-col justify-start py-4'>
        <div onClick={()=> navigate('/')} className='flex items-center gap-3 pl-8 py-2 cursor-pointer'>
          <img className='w-6' src={assets.home_icon} alt="Home icon" />
          <p className='font-bold'>Home</p>
        </div>
        <div onClick={() => navigate('/search')} className='flex items-center gap-3 pl-8 py-2 cursor-pointer'>
      ...
      <div className='bg-[#121212] h-[85%] rounded'>
        ...
        <div className='p-4 bg-[#242424] m-2 rounded font-semibold flex flex-col items-start justify-start gap-1 pl-4 mt-4'>
          ...
          <button onClick={() => navigate('/browse-podcasts')} className='px-4 py-1.5 bg-white text-[15px] text-black rounded-full mt-4'>
            Browse podcasts
          </button>
        </div>
      </div>
